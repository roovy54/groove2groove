...
mkdir "$dir" && {
  ...
  zcat 02_chopped/data_meta.json.gz | python -c '
import json, sys
...
json.dump(data_dict, sys.stdout, separators=(",", ":"))
  ...
  # Add the reformat python function here
  python -c '
import sys

def reformat_key_map_file(file_path):
    with open(file_path, "r") as file:
        lines = file.readlines()

    with open(file_path, "w") as file:
        for i in range(0, len(lines) - 1, 2):
            src_key = lines[i].strip()
            tgt_key = lines[i+1].strip()
            combined_line = f"{src_key}\t{tgt_key}\n"
            file.write(combined_line)
            print(f"Processed: {combined_line.strip()}")

    print(f"\nFile has been reformatted in-place: {file_path}")

if __name__ == "__main__":
    key_map_file_path = sys.argv[1]
    reformat_key_map_file(key_map_file_path)
' "$dir/shuf/key_map" || die
